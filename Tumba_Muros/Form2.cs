...
namespace Tumba_Muros
{
    public partial class escenario2 : Form
    {
        ...
        GestionDB GestionDB = new GestionDB(); // GestionDB para agregar el puntaje a la db
        ...
        private void escenario2_Load(object sender, EventArgs e)
        {
            paleta = new Paleta("paleta2", 260, 410, 60, 25); // Crear una instancia de Paleta
            this.Controls.Add(paleta.ImagenObjeto); // la agrega
            pelota = new Pelota("pelota2", 273, 350, 30, 30); // Crear una instancia de pelota
            this.Controls.Add(pelota.ImagenObjeto); // la agrega
            CargarMuros(); // Carga los muros
            CargarLadrillos(); // Carga los ladrillos

            audio.SeleccionarAudio(5);
        }
        ...
        public void CargarLadrillos()
        {
            ...
            {
                Ladrillo ladrillo = new Ladrillo(coor[i, 0], coor[i, 1], "ladrillo2");
        ...
        public void CargarMuros()
        {
            ...
            for (int i = 0; i < coor.GetLength(0); i++)
            {
                Muro muro = new Muro(coor[i, 0], coor[i, 1], "barrera2");
                Muros.Add(muro);
        ...
        private void timerPelota_Tick(object sender, EventArgs e)
        {
            ...
            if (jugador.Vidas1 == 0)
            {
                ...
                GestionDB.AgregarPuntaje(jugador);
        ...
        public void RecompensaAleatoria()
        {
            if (!recompensaCreada)
            {
                ...
                int posX = new Random().Next(70, 400); //posicion de la recompensa aleatoria
                ...
                recompensaCreada = true; // devuelve que la recompensa fue creada
            }
        }
        ...
        private void InstanciaYControl(int posX, int seg)
        {
            ...
            {
                Vida = new ObjetoGrafico("vida2", posX, 55, 30, 30); // Crear la vida en la posición inicial
        ...
        {
            InstanciaYControl(100, 30); //crea la vida y hace el proceso para el aumento de la vida si es que la alcanza a tocar con la paleta
            InstanciaYControl(400, 60);
        }
        ...
        private void Mover(ref ObjetoGrafico objeto, bool esVida)
        {
            if (objeto != null)
            {
                ...
                if (jugador.GanarRecompensa(paleta, objeto))
                {
                    ...
                    this.Controls.Remove(objeto.ImagenObjeto); // Eliminar el objeto del control
        ...
        private void timerRecompensa_Tick(object sender, EventArgs e)
        {
            ...
            // Si el puntaje es de 4500 o mas gana el juego
            if (jugador.Puntaje1 >= 4500)
            {
                // Se muestra el mensaje de que gano y suena
                Mensaje mensaje = new Mensaje();
                audio.SeleccionarAudio(6);
                mensaje.Estado = 1;
                mensaje.Show();
